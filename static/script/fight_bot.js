...
function resetSuggestion() {
    ...
    .then(res => res.json())
    .then(data => {
        Array.from(players).forEach((warrior) => {
            warrior.remove()
        })
        console.log(players)
        data.forEach(user => {
            playerList.innerHTML += `
                <li class="player" data-name=${user[0]} data-elo=${user[1]}>
                    <div class="player-avatar"><p>${user[0][0].toUpperCase()}</p></div>
                    <div class="player-info">
                    <div class="player-info--name">${user[0]}</div>
                    <div class="player-info--elo">${user[1]}</div>
                    </div>
                </li>
            `
        })
